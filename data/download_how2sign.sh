...
# If no arguments are provided, use a sensible default set of modalities
# (sentence-level front clips + re-aligned English translations)
if (( $# < 1 )); then
    echo "No modalities specified."
    echo "Using default: rgb_front_clips english_translation_re-aligned"
    set -- rgb_front_clips english_translation_re-aligned
fi
